...
const Login: React.FC = () => {
  ...
  }, [loginMutation.status, loginMutation.error]);

  const onSubmit: SubmitHandler<LoginSchema> = async (e) => {
    const data = {
      email: e.email,
      password: e.password,
    };
      ...
      <div className="flex w-full items-start justify-center mb-10 sm:mb-20 px-4 sm:px-0">
        ...
        <section className="w-full max-w-[350px] sm:w-[20%] h-full mt-4">
          <form onSubmit={handleSubmit(onSubmit)} className="w-full space-y-2 border border-[#555555] px-4 sm:px-10">
            <h1 className="flex justify-center text-3xl sm:text-4xl py-8 sm:py-10 text-white">Instagram</h1>
            ...
            <Button type="submit" className="w-full bg-[#0069AD] text-white py-2 mt-2 mb-6 text-sm rounded-md">
            ...
            <div className="flex flex-col space-y-4 w-full max-w-xs mx-auto">
              ...
              <div className="flex items-center gap-4">
                ...
                <span className="text-[#737373] text-xs sm:text-[13px] font-semibold">OR</span>
                <div className="flex-1 h-px bg-[#555555]"></div>
              </div>
              ...
              <Link to={`${ROUTES.auth}/${ROUTES.forgotPassword}`} className="text-center text-sm sm:text-base text-white mb-4">
                Forgot password?
              </Link>
            </div>
          </form>
          <div className="text-center justify-center flex gap-1.5 p-4 border text-xs sm:text-sm border-[#555555] mt-3">
            ...
            <Link to={`${ROUTES.auth}/${ROUTES.signup}`} className="text-[#0095F6]">
              Sign Up
            </Link>
          </div>
          <div className="flex flex-col items-center justify-center gap-4 p-4">
            <h4 className="text-center text-sm sm:text-base text-[#F5F5F5]">Get the app.</h4>
